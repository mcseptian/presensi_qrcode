...
<script type="text/javascript">
    ...
    $("#btn_create_pertemuan").click(function () {
        ...
        if (kelompok == 'null') { // jika belum memilih kelompok
            $("#tempat_box_dialog").load("<?php echo site_url("dialog/error_pertemuan"); ?>"); //munculkan dialog error
        } else {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url("dialog/kepastian_pertemuan"); ?>", //munculkan dialog verifikasi
    ...
    function open_password(tujuan) {
        ...
        setTimeout(function () {
            ...
            $("#tempat_box_dialog").load("<?php echo site_url("dialog/buka_password"); ?>/" + tujuan);
        }, 0400);
    }
    ...
    function input_pertemuan() {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url("pertemuan/buat_pertemuan"); ?>",
            data: $("#form_pertemuan").serialize(),
            success: function (result) {
                window.location.replace("<?php echo site_url("absensi"); ?>")
            }
        });
    }
    ...
    function cek_password(tujuan) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url("dialog/cek_password"); ?>",
            data: "username=" + pass,
            success: function (result) {
                if (result == 1) {
                    ...
                    setTimeout(function () {
                        ...
                        $("#tempat_box_dialog").load("<?php echo site_url("dialog/salah_password"); ?>");
                    }, 0300);
                }
            }
        });
    ...
    function proses_absen() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url("absensi/proses_absen"); ?>",
